...
export function buildHarnessDeveloperInstructions(
  ...
  return [
    ...
    "available_context.approved_public_urls are public HTTP(S) message URLs admitted by System. available_context.public_source_resources describe canonical shared resources, readable_public_url_candidates describe retrieval attempts for those resources, public_source_facts contain fetched public text, and public_source_failures contain failed attempts. blocked_urls are visible context, not approved fetch targets.",
    "For public_source_facts, canonical_item_url identifies the shared item and retrieval_url identifies the concrete fetch endpoint that produced the fact. Use fact text/title as source facts when present, and use public_source_failures only to explain evidence limits when relevant.",
    "For an X/Twitter status URL such as x.com/{handle}/status/{id} or twitter.com/{handle}/status/{id}, treat the status as a typed evidence resource. System prepares readable_public_url_candidates for the same resource, such as https://api.fxtwitter.com/2/status/{id} or https://r.jina.ai/https://x.com/{handle}/status/{id}; these candidates are retrieval-route facts, not an instruction for you to implement a provider order.",
    "For X/Twitter public_source_facts, fact.canonical_item_url is the x.com status URL and fact.retrieval_url is the concrete readable candidate that produced the text. Prefer available_context.public_source_facts when System has already fetched the readable post body.",
    ...
    "If available_context.place_context.features includes knowledge_ingest, available_context.thread_context.kind is root_channel, and capabilities.allow_external_fetch is true, do not stop at approved_public_urls when available facts are absent or too thin to identify the shared content. Use the approved URL as a lead, then investigate closely related public sources needed to identify and summarize that same shared item.",
    "When you broaden from approved_public_urls in a knowledge_ingest root context, keep the research tightly anchored to the specific shared post, article, video, release, or announcement. Do not drift into general background research unless the user explicitly asks for it.",
    ...
    "If you need to establish same-turn public reconfirmation for a public URL that is not already in approved_public_urls or public_source_facts, use the repo skill public-source-fetch and its read-only script. Only structured public-source-fetch output with non-empty title or text establishes observed public source evidence for System.",
    ...
    "knowledge_writes are advisory persistence handoff. Missing or partial knowledge_writes should not block a successful answer. Every knowledge_write with summary or normalized_text must include evidence_fact_ids for public_source_facts whose text is non-empty.",
    "If outcome is knowledge_ingest, do not persist a summary or normalized_text without public_source_facts evidence. If no fact text exists, answer normally without knowledge_writes or state that the source could not be read.",
    "If outcome is knowledge_ingest and you want to cite or persist a public URL that is not already in approved_public_urls or public_source_facts, do not include that URL in sources_used or knowledge_writes unless public-source-fetch established same-turn public source evidence for it with non-empty title or text.",
