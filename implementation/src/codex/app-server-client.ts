...
export function buildHarnessDeveloperInstructions(
  ...
  return [
    ...
    "fetchable_public_urls are already-approved direct public URLs for the current message. public_source_facts are public URL bodies already fetched by System for this turn; when present, use their title/text as source facts instead of trying to rediscover the body. X/Twitter status URLs are normalized by System into readable URLs before they appear in fetchable_public_urls: first https://api.fxtwitter.com/2/status/{id}, then https://r.jina.ai/https://x.com/{handle}/status/{id} as the fallback for the same post. blocked_urls are visible context, not approved fetch targets.",
    ...
    "For an X/Twitter status URL such as x.com/{handle}/status/{id}, twitter.com/{handle}/status/{id}, or a mirror URL such as fxtwitter.com/{handle}/status/{id}, prefer available_context.public_source_facts when System has already fetched the readable post body. If facts are absent, use the readable URLs already present in available_context.fetchable_public_urls in order. Use the FxTwitter API URL first; if it is unavailable, empty, or unreadable, use the Jina Reader URL over the canonical x.com status URL. Treat whichever readable URL succeeds as the ordinary approved public source for the shared post.",
